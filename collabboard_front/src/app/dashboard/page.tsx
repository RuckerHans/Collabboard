  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto max-w-6xl px-6 py-8">
        ...
        {error && <p role="alert" className="mt-5 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{getApiErrorMessage(error, 'Boards could not be loaded.')}</p>}
      ...
      {open && <CreateBoardModal onClose={() => { createBoard.reset(); setOpen(false); }} onCreate={(values) => createBoard.mutate(values, { onSuccess: () => setOpen(false) })} pending={createBoard.isPending} error={createBoard.error} />}
    </main>
  );
}
...
function CreateBoardModal({ onClose, onCreate, pending, error }: { onClose: () => void; onCreate: (values: { name: string; description?: string }) => void; pending: boolean; error: unknown }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/30 p-4"><form onSubmit={(event) => { event.preventDefault(); onCreate({ name, description: description || undefined }); }} className="w-full max-w-md rounded-lg bg-white p-6 shadow-panel"><h2 className="text-xl font-semibold">Create board</h2>{Boolean(error) && <p role="alert" className="mt-4 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{getApiErrorMessage(error, 'The board could not be created.')}</p>}<input value={name} onChange={(e) => setName(e.target.value)} required maxLength={120} placeholder="Board name" className="mt-5 w-full rounded-md border border-line px-3 py-2" /><textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" className="mt-3 h-24 w-full rounded-md border border-line px-3 py-2" /><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-md border border-line px-4 py-2">Cancel</button><button disabled={pending} className="rounded-md bg-brand-600 px-4 py-2 font-semibold text-white disabled:opacity-50">{pending ? 'Creating...' : 'Create'}</button></div></form></div>;
}
