...
export function useBoard(boardId: string) {
  ...
  const deleteBoard = useMutation({ mutationFn: async () => (await api.delete(`/boards/${boardId}`)).data });
...
export function useNotes(boardId: string) {
  ...
  const createNote = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['notes', boardId] }),
  });
  ...
  const deleteNote = useMutation({ mutationFn: async (id: string) => (await api.delete(`/boards/${boardId}/notes/${id}`)).data });
  const restoreNote = useMutation({ mutationFn: async (id: string) => (await api.post<Note>(`/boards/${boardId}/notes/${id}/restore`)).data });
